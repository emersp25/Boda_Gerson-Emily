...
(function initParticles() {
  ...
  const PALETTE = [
    'rgba(201,168,76,', 'rgba(226,200,122,',
    'rgba(240,224,168,', 'rgba(255,253,246,', 'rgba(139,105,20,',
  ];
  ...
  function drawStar(ctx, x, y, r) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      i === 0 ? ctx.moveTo(x + Math.cos(ang)*len, y + Math.sin(ang)*len)
  ...
  function drawDiamond(ctx, x, y, r) {
    ctx.beginPath();
    ctx.moveTo(x, y - r); ctx.lineTo(x + r*.55, y);
    ctx.lineTo(x, y + r); ctx.lineTo(x - r*.55, y);
    ctx.closePath(); ctx.fill();
  }

  function buildParticles(n) { particles = Array.from({length:n}, () => { const p = new Particle(); p.life = Math.random(); return p; }); }
  ...
  function loop(ts) {
    ...
    for (const p of particles) {
      ...
      if (p.type === 0) {
        ctx.shadowBlur = p.r * 6; ctx.shadowColor = p.col + '.6)';
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i+1; j < particles.length; j++) {
        ...
        if (dist < maxDist) {
          ctx.strokeStyle = `rgba(201,168,76,${(1 - dist/maxDist) * .04})`;
          ctx.lineWidth = .5;
  ...
  function count() { return window.innerWidth < 600 ? 45 : 80; }
  window.addEventListener('resize', () => { resize(); buildParticles(count()); });
  resize(); buildParticles(count());
  requestAnimationFrame(loop);
})();
...
(function initPetals() {
  ...
  const COLORS = ['rgba(201,168,76,.55)','rgba(226,200,122,.45)','rgba(240,224,168,.5)','rgba(255,253,246,.65)'];
  ...
  function createPetal() {
    ...
    el.addEventListener('animationend', () => { el.remove(); createPetal(); });
  }
...
function goTo(idx) {
  ...
  setTimeout(() => {
    document.querySelectorAll('.slide.exit-left').forEach(s => s.classList.remove('exit-left'));
    animating = false;
  }, 700);
}
...
document.addEventListener('touchend', e => {
  ...
  if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 45) goTo(dx < 0 ? current+1 : current-1);
}, { passive:true });
...
/* ══════════════════════════════════════
   CUENTA REGRESIVA
══════════════════════════════════════ */
const WEDDING = new Date('2026-05-09T15:00:00-06:00');
const [elD, elH, elM, elS] = ['c-days','c-hours','c-minutes','c-seconds'].map(id => document.getElementById(id));

function pad2(n) { return String(n).padStart(2, '0'); }

function tick(el) { el.classList.remove('tick'); void el.offsetWidth; el.classList.add('tick'); setTimeout(() => el.classList.remove('tick'), 250); }

function updateCountdown() {
  const diff = WEDDING - Date.now();
  if (diff <= 0) { [elD,elH,elM,elS].forEach(e => e && (e.textContent='00')); return; }
...
async function submit(asiste) {
  ...
  setBtns(true);
  try {
    await fetch(APPS_SCRIPT_URL, { method:'POST', mode:'no-cors', body: new URLSearchParams({familia, cantidad, asiste}) });
    showSuccess(familia, asiste);
  } catch(e) {
    console.error(e); setBtns(false);
